Gyarabium Core version *0.15.1* is now available from:

  <https://download.gyarabium.org/gyarabium-0.15.1/>
...
A number of changes to the way Gyarabium Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 gyarabons per kB (0.00001 GRB). This is to prepare for dropping the minimum transaction fee to 0.1 gyarabons per kB (0.0001 GRB) in the next release.
